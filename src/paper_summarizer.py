...
- **纯粹的“左脚踩右脚”式实验**：指仅通过大模型自我生成、自我评估、自我循环，而未对底层数据本质或系统机制进行深入探究的“玩闹式”实验。
- **缺乏实质性发现**：未能揭示导致策略有效的关键信息或新的信息挖掘机制。
...
2. **拒绝盲目实验**：在评估 technical_innovation 和 decision 时，**优先挖掘论文是否触及了数据本质**。若论文仅展现了模型指标的提升，却无法解释是何种关键信息或系统机制导致了策略有效，应审慎给出“推荐”。
3. **通俗易懂**：用业界通用的技术语言，而不是晦涩的数学描述。
4. **技术创新详细化**：technical_innovation 字段必须详细描述论文Method部分的内容，使用编号列表格式，每个技术点都要说明具体的方法、算法、流程、数据规模或实验结果。
5. **保持简短**：总结篇幅严格控制在 200 字以内，应用价值限制在88字以内，技术创新限制在200字以内，决策理由限制在150字以内。
6. **只输出 JSON**：不要添加任何解释、注释或额外文字，确保所有输出符合严格的 JSON 格式。
7. **严格转义**：确保输出的 JSON 字符串中，所有字段值内的双引号必须使用反斜杠转义（如 \"内容\"），禁止出现原始的换行符。
8. **结构完整**：输出必须是一个完整的 JSON 数组，确保每篇论文对象之间有逗号分隔。
9. **决策准确**：decision 字段只能是"推荐"、"边缘可看"、"不推荐"三者之一，不要有其他文字。
